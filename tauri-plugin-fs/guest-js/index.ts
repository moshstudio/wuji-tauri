...
class FileHandle extends Resource {
  ...
  async read(buffer: Uint8Array): Promise<number | null> {
    ...
    const data = await invoke<ArrayBuffer | number[]>('plugin:fs2|read', {
      rid: this.rid,
  ...
  async seek(offset: number, whence: SeekMode): Promise<number> {
    return await invoke('plugin:fs2|seek', {
      rid: this.rid,
      offset,
      whence,
    });
  ...
  async stat(): Promise<FileInfo> {
    const res = await invoke<UnparsedFileInfo>('plugin:fs2|fstat', {
      rid: this.rid,
    });
  ...
  async truncate(len?: number): Promise<void> {
    await invoke('plugin:fs2|ftruncate', {
      rid: this.rid,
      len,
    });
  }
  ...
  async write(data: Uint8Array): Promise<number> {
    return await invoke('plugin:fs2|write', {
      rid: this.rid,
      data,
    });
  }
...
async function create(
  ...
  const rid = await invoke<number>('plugin:fs2|create', {
...
async function open(
  ...
  const rid = await invoke<number>('plugin:fs2|open', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
...
async function copyFile(
  ...
  await invoke('plugin:fs2|copy_file', {
...
async function readDir(
  ...
  return await invoke('plugin:fs2|read_dir', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
}
...
async function readFile(
  ...
  const arr = await invoke<ArrayBuffer | number[]>('plugin:fs2|read_file', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
...
async function readTextFileLines(
  ...
  return await Promise.resolve({
    ...
    async next(): Promise<IteratorResult<string>> {
      if (this.rid === null) {
        this.rid = await invoke<number>('plugin:fs2|read_text_file_lines', {
          path: pathStr,
          options,
        });
      }
      ...
      const arr = await invoke<ArrayBuffer | number[]>(
        'plugin:fs2|read_text_file_lines_next',
        { rid: this.rid }
      );
...
async function remove(
  ...
  await invoke('plugin:fs2|remove', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
}
...
async function rename(
  ...
  await invoke('plugin:fs2|rename', {
...
async function stat(
  ...
  const res = await invoke<UnparsedFileInfo>('plugin:fs2|stat', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
...
async function lstat(
  ...
  const res = await invoke<UnparsedFileInfo>('plugin:fs2|lstat', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
...
async function truncate(
  ...
  await invoke('plugin:fs2|truncate', {
    path: path instanceof URL ? path.toString() : path,
    len,
    options,
  });
...
async function writeFile(
  ...
  if (data instanceof ReadableStream) {
    ...
    await invoke('plugin:fs2|write_file', data, {
      headers: {
...
async function writeTextFile(
  ...
  await invoke('plugin:fs2|write_text_file', encoder.encode(data), {
    headers: {
      path: encodeURIComponent(path instanceof URL ? path.toString() : path),
      options: JSON.stringify(options),
    },
...
async function exists(
  ...
  return await invoke('plugin:fs2|exists', {
    path: path instanceof URL ? path.toString() : path,
    options,
  });
}
...
async function unwatch(rid: number): Promise<void> {
  await invoke('plugin:fs2|unwatch', { rid });
}
...
async function watch(
  ...
  const rid: number = await invoke('plugin:fs2|watch', {
...
async function size(path: string | URL): Promise<number> {
  ...
  return await invoke('plugin:fs2|size', {
    path: path instanceof URL ? path.toString() : path,
  });
}
...
export async function fetchAndSave(
  ...
  const res = await invoke<boolean>('plugin:fs2|fetch_and_save', {
    options: {
      method: req.method,
      url: req.url,
