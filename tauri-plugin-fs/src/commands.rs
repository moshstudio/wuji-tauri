...
use tokio::fs as tokio_fs; // 为 tokio::fs 起一个别名
...
) -> CommandResult<reqwest::ClientBuilder> {
    let Proxy { all, http, https } = proxy;

    if let Some(all) = all {
        let proxy =
            proxy_creator(all, reqwest::Proxy::all).map_err(|e| format!("all proxy error: {e}"))?;
        builder = builder.proxy(proxy);
    }

    if let Some(http) = http {
        let proxy = proxy_creator(http, reqwest::Proxy::http)
            .map_err(|e| format!("http proxy error: {e}"))?;
        builder = builder.proxy(proxy);
    }

    if let Some(https) = https {
        let proxy = proxy_creator(https, reqwest::Proxy::https)
            .map_err(|e| format!("https proxy error: {e}"))?;
        builder = builder.proxy(proxy);
    }
...
fn build_request(
    mut builder: reqwest::ClientBuilder,
    client_config: ClientSaveConfig,
) -> CommandResult<reqwest::RequestBuilder> {
    let ClientSaveConfig {
        method,
        url,
        headers: headers_raw,
        data,
    ...
    match scheme {
        "http" | "https" => {
            ...
            for (h, v) in headers_raw {
                let name =
                    http::HeaderName::from_str(&h).map_err(|e| format!("wrong head name: {e}"))?;
                headers.append(
                    name,
                    http::HeaderValue::from_str(&v)
                        .map_err(|e| format!("wrong head value: {e}"))?,
                );
            }
            ...
            if let Some(timeout) = connect_timeout {
                builder = builder.connect_timeout(std::time::Duration::from_millis(timeout));
            }
            ...
            let mut request = builder
                .build()
                .map_err(|e| format!("reqest build failed: {e}"))?
                .request(method.clone(), url_clone);

            // POST and PUT requests should always have a 0 length content-length,
            // if there is no body. https://fetch.spec.whatwg.org/#http-network-or-cache-fetch
            if data.is_none() && matches!(method, reqwest::Method::POST | reqwest::Method::PUT) {
                headers.append(
                    reqwest::header::CONTENT_LENGTH,
                    http::HeaderValue::from_str("0")
                        .map_err(|e| format!("add head 0 failed: {e}"))?,
                );
            }
            ...
            if headers.contains_key(reqwest::header::RANGE) {
                ...
                headers.append(
                    ...
                    http::HeaderValue::from_str("identity")
                        .map_err(|e| format!("add accept_encoding identity failed: {e}"))?,
                );
            }
            ...
            if !headers.contains_key(reqwest::header::USER_AGENT) {
                ...
                if let Some(random_ua) = STATIC_CHROME_AGENTS.choose(&mut rng) {
                    headers.append(
                        ...
                        http::HeaderValue::from_str(random_ua)
                            .map_err(|e| format!("random ua failed: {e}"))?,
                    );
                }
            }
        ...
        "data" => {
            ...
            // Ok(rid)
            Err(CommandError::SchemeNotSupport(scheme.to_string()))
        }
        _ => Err(CommandError::SchemeNotSupport(scheme.to_string())),
    }
}
...
pub async fn fetch_and_save<R: Runtime>(
    ...
    let request = build_request(builder, options)?;
    ...
    if data.is_empty() {
        ...
        let path = resolve_path(&webview, &global_scope, &command_scope, safe_path, base_dir)?;
        println!("save path {:?}", path);
        let mut file = std::fs::OpenOptions::from(crate::OpenOptions {
            read: false,
            write: true,
            truncate: true,
            create: true,
        ...
        file.write_all(&data)
            ...
            .map_err(|e| {
                format!(
                    "failed to write file to path: {} with error: {e}",
                    path.display()
                )
            })?;
...
pub async fn write_file<R: Runtime>(
    ...
    let data = match request.body() {
        ...
        _ => return Err(anyhow!("unexpected invoke body").into()),
    };

    let path = request
        .headers()
        .get("path")
        .ok_or_else(|| anyhow!("missing file path").into())
        .and_then(|p| {
            percent_decode(p.as_ref())
                .decode_utf8()
                .map_err(|_| anyhow!("path is not a valid UTF-8").into())
        })
    ...
    // // 使用 tokio_fs::File 替代 std::fs::File
