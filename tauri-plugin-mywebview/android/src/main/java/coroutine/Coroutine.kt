...
class Coroutine<T>(
        ...
        cancel?.let {
            DEFAULT.launch(executeContext) {
                if (null == it.context) {
                    ...
                    withContext(it.context) { it.block.invoke(this) }
                }
            }
        }
    }
    ...
    private suspend inline fun dispatchVoidCallback(scope: CoroutineScope, callback: VoidCallback) {
        if (null == callback.context) {
            withContext(scope.coroutineContext) { callback.block.invoke(scope) }
        } else {
            withContext(scope.coroutineContext + callback.context) { callback.block.invoke(this) }
        }
    }
