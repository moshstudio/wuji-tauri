...
形态判断后仍须按 §5.6 **多试几种情况**（PC/手机、代理/直连、多套选择器），不要只认第一种能打开的页面。
...
- `this.fetch` **没有** `timeout`（那是 `fetchWebview` 的秒级参数）；限时用 `connectTimeout`（毫秒，默认 15s）
- `Set-Cookie` 不在 `headers.get('set-cookie')` 里，必须用 `headers.getSetCookie()`
- 站点拦代理 IP 时会 403：所有请求加 `noProxy: true`（用户明确说「不使用代理」时必须如此）

### 5.6 鲁棒性：多试几种情况（强制）

源面对的是活站点：PC/手机版、首页/分类/排行、短篇/长篇、有无分页、有无代理，结构经常不一致。  
**禁止只按一种 HTML / 一种请求方式写死。** 制作或修复时必须自己抓真实页面，对每种入口各试几种路径，主路径失败再走兜底。

#### 制作/修复时必测的页面

每个抽象方法至少用 **2 个以上真实 URL** 验证，而不是只看首页：

| 方法 | 至少覆盖 |
|------|----------|
| 推荐/列表 | 首页、一个分类、一个排行/完本（选择器常不同，如 `#main article` vs `#list article`） |
| 搜索 | 能命中的书名/作者、无结果关键词；若有频控，要能重试而不是抛错 |
| 详情 | 短书（几十章）+ 长书（上千章，看目录是否分页） |
| 正文/图片/播放 | 普通一章 + 可能「下一页」的长章；**不要把「下一章」当成「下一页」** |

视频还要：至少两条线路、一集有地址/一集需嗅探、直播 vs 点播（若站点两者都有）。

#### 请求层：按下面顺序试，直到拿到有效数据

1. **直连 vs 代理**：先普通 `fetch`；若 403/空页/status=0，改 `noProxy: true`（或反过来）。证书问题时加 `verify: false`。
2. **UA / Referer**：移动站用 Mobile UA，PC 站用桌面 UA；封面和正文页带站点 `Referer`。
3. **编码**：UTF-8 乱码再试 `fetchDom(..., 'text/html', 'gbk')`。
4. **Cookie**：搜索/签名接口先 GET 业务页再 POST；Cookie 用 `getSetCookie()` 拼 `SID=...`，不要依赖 `headers.get('set-cookie')`。
5. **DOM vs API vs Webview**：静态 HTML → `fetchDom`；XHR/JSON → `fetch`；必须执行 JS / 要嗅探媒体 → `fetchWebview`。一种拿不到就换，不要认定站点挂了。

#### 解析层：选择器与字段都要有备选

- 列表容器、标题、链接、封面、章节、正文 **各准备 2 套选择器**；第一套 `query*Elements` / `queryChapters` 为空再换第二套。
- 链接选择器宁可偏严（如必须带封面的 `.image a`），避免把分类块、导航、热门推荐收进列表。
- 正文容器优先具体节点（`#text`、`#htmlContent`），避免过宽的 `#content`（可能是 `body` 或包了整页）。
- 字段兜底：`item.url || item.id`；`title || book_name`；`og:` meta 补简介/作者/封面/状态。
- 清洗前缀：`作者：`、`简介：` 等。
- 分页循环必须用 `Set` 去重，防止「下一页」指回当前章。

#### 失败时的返回

- 单次解析失败返回空列表 / `null`，**不要抛异常**（框架虽会兜底，源内仍应自己收口）。
- 搜索鉴权成功但 `list` 为空：保留实现，让用户能从推荐/分类进书；不要为了「看起来能搜」去扫全站。
- 频控（如「搜索太频繁」）可短等后重试 **一次**，不要死循环。

#### 交付前自检（缺一不可）

对照真实响应确认：推荐至少一个 Tab 有书、详情章节数量合理、正文/图片/播放地址非空且不是导航残渣。只截一张页面、只测一条 URL **不算完成**。
...
8. **鲁棒**：请求参数、选择器、正文/章节容器按 §5.6 做备选与空结果兜底；用户指定 `noProxy` 时所有 `fetch`/`fetchDom` 必须带上
---
...
- 参考的其他软件源规则：（如有）
- 抓包/JSON 样例：（如有）
- 特殊要求：（登录、仅搜索、过滤分类、不使用代理等）

输出完整 JS 类代码，以 return CustomXxxExtension 结尾。
要求：代码简洁规范；_ 开头辅助函数放在 constructor 之后、对外方法之前。
鲁棒性（§5.6）：先实际抓首页/分类/排行/搜索/详情/内容（含长篇），
请求层依次试 代理/noProxy、UA、编码、Cookie(getSetCookie)、DOM/API/webview；
解析层每类字段至少两套选择器，空列表再换；正文不要误跟「下一章」；
验证过再交付，禁止只按一种 HTML 写死。
```
...
*文档版本：与仓库 `packages/source-extension` 同步，更新日期 2026-09-07*
